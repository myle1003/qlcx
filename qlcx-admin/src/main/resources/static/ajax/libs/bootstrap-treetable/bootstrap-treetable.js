    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        var initToolbar = function() {
            ...
            if (options.showSearch) {
                var $searchBtn = $('<button class="btn btn-default btn-outline" type="button" aria-label="search" title="Tìm kiếm"><i class="glyphicon glyphicon-search"></i></button>');
                $rightToolbar.append($searchBtn);
                registerSearchBtnClickEvent($searchBtn);
            }
            // 是否显示Chuyển按钮
            if (options.showRefresh) {
                var $refreshBtn = $('<button class="btn btn-default btn-outline" type="button" aria-label="refresh" title="Chuyển"><i class="glyphicon glyphicon-repeat"></i></button>');
        ...
        var initServer = function(parms) {
            ...
            //Thêm vào加载loading
            var $loading = '<tr><td colspan="' + options.columns.length + '"><div style="display: block;text-align: center;">Đang tải dữ liệu, vui lòng đợi...</div></td></tr>'
            $tbody.html($loading);
            if (options.url) {
                $.ajax({
                    type: options.type,
        ...
        var renderTable = function(data) {
            ...
            if (!data || data.length <= 0) {
                var _empty = '<tr><td colspan="' + options.columns.length + '"><div style="display: block;text-align: center;">Không tìm thấy kết quả</div></td></tr>'
                $tbody.html(_empty);
                return;
            }
            // 缓存并格式化数据
            ...
            // 下边的Action主要是为了查询时让一些没有根节点的节点显示
        ...
        var registerExpanderEvent = function() {
            ...
            target.find("tbody").find("tr").find(".treetable-expander").click(function() {
                ...
                if (_isExpanded || _isCollapsed) {
                    ...
                    var _ls = target.find("tbody").find("tr[id^='" + row_id + "_']"); //下tất cả
                    if (_isExpanded) {
