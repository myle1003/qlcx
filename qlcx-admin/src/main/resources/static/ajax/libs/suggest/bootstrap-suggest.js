 ...
 * @desc    这是一个基于 bootstrap 按钮式下拉菜单组件的Tìm kiếm建议插件，必须使用于按钮式下拉菜单组件上。
...
(function (factory) {
    ...
    var inputLock; // 用于中文输入法输入时锁定Tìm kiếm
    ...
    /**
     * 判断字段名是否在 options.searchFields Tìm kiếm字段配置中
     */
    ...
    function isInWord(keyword, key, value, options) {
        ...
        return value &&
            (inEffectiveFields(key, options) || inSearchFields(key, options)) && // 必须在有效的Tìm kiếm字段中
            (
    ...
    var defaultOptions = {
        ...
        allowNoKeyword: TRUE,           // 是否允许Không关键字时请求数据
        getDataMethod: 'firstByUrl',    // 获取数据的方式，url：一直从url请求；data：从 options.data 获取；firstByUrl：第一次从Url获取全部数据，之后从options.data获取
        delayUntilKeyup: FALSE,         // 获取数据的方式 为 firstByUrl 时，是否延迟到有输入时才请求数据
        ignorecase: FALSE,              // 前端Tìm kiếm匹配时，是否忽略大小写
        effectiveFields: [],            // 有效显示于Danh sách中的字段，非有效字段都会过滤，默认全部有效。
        effectiveFieldsAlias: {},       // 有效字段的别名对象，用于 header 的显示
        searchFields: [],               // 有效Tìm kiếm字段，从前端Tìm kiếm过滤数据时使用，但不一定显示在Danh sách中。effectiveFields 配置字段也会用于Tìm kiếm过滤
        twoWayMatch: TRUE,              // 是否双向匹配Tìm kiếm。为 true 即输入关键字包含或包含于匹配字段均认为匹配成功，为 false 则输入关键字包含于匹配字段认为匹配成功
        multiWord: FALSE,               // 以分隔符号分割的多关键字支持
        separator: ',',                 // 多关键字支持时的分隔符，默认为半角逗号
        delay: 300,                     // Tìm kiếm触发的延时时间间隔，单位毫秒
        emptyTip: '',                   // 查询为空时显示的内容，可为 html
        searchingTip: 'Tìm kiếm中...',       // ajax Tìm kiếm时显示的提示内容，当Tìm kiếm时间较长时给出正在Tìm kiếm的提示
        hideOnSelect: FALSE,            // 鼠标从Danh sách单击选择了值时，是否隐藏选择Danh sách

        /* UI */
        autoDropup: FALSE,              // 选择菜单是否自动判断向上展开。设为 true，则当下拉菜单高度超过窗体，且向上方向不会被窗体覆盖，则选择菜单向上弹出
        autoMinWidth: FALSE,            // 是否自动最小宽度，设为 false 则最小宽度不小于输入框宽度
        showHeader: FALSE,              // 是否显示选择Danh sách的 header。为 true 时，有效字段大于一列则显示表头
        showBtn: TRUE,                  // 是否显示下拉按钮
        inputBgColor: '',               // 输入框背景色，当与容器背景色不同时，可能需要该项的配置
        inputWarnColor: 'rgba(255,0,0,.1)', // 输入框内容不是下拉Danh sách选择时的警告色
        listStyle: {
            'padding-top': 0,
            'max-height': '375px',
            'max-width': '800px',
        ...
        listAlign: 'left',              // 提示Danh sách对齐位置，left/right/auto
        listHoverStyle: 'background: #07d; color:#fff', // 提示框Danh sách鼠标悬浮的样式
        listHoverCSS: 'jhover',         // 提示框Danh sách鼠标悬浮的样式名称
        clearable: FALSE,               // 是否可清除已输入的内容

        /* key */
        keyLeft: 37,                    // 向左方向键，不同的Action系统可能会有差别，则自行定义
        keyUp: 38,                      // 向上方向键
        keyRight: 39,                   // 向右方向键
        keyDown: 40,                    // 向下方向键
        keyEnter: 13,                   // 回车键
        ...
        fnGetData: getData,             // 获取数据的方法，Không特殊需求一般不作设置
        fnAdjustAjaxParam: null,        // 调整 ajax 请求参数方法，用于更多的请求配置需求。如对请求关键字作进一步处理、修改超时时间等
        fnPreprocessKeyword: null,      // Tìm kiếm过滤数据前，对输入关键字作进一步处理方法。注意，应返回字符串
    ...
    var methods = {
        init: function(options) {
            ...
            // 默认配置有效显示字段多于一个，则显示Danh sách表头，否则不显示
            ...
            return self.each(function() {
                ...
                $input.on('keydown', function(event) {
                    var currentList, tipsKeyword; // 提示Danh sách上被选中的关键字
                    ...
                    if (!isMouseenterMenu) { // 不是进入下拉Danh sách状态，则隐藏Danh sách
                        hideDropMenu($input, options);
                    }
                }).on('click', function() {
