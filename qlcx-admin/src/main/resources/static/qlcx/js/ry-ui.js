  ...
  $.extend({
    ...
    table: {
      init: function (options) {
        ...
        $("#" + options.id).bootstrapTable({
          ...
          search: options.search, // 是否显示Tìm kiếm框功能
          searchText: options.searchText, // Tìm kiếm框初始显示的内容，默认为空
          showSearch: options.showSearch, // 是否显示检索信息
          showPageGo: options.showPageGo, // 是否显示Nhảy页
          showRefresh: options.showRefresh, // 是否显示Chuyển按钮
          showColumns: options.showColumns, // 是否显示隐藏某列下拉框
          showToggle: options.showToggle, // 是否显示详细视图和Danh sách视图的Cột按钮
          showExport: options.showExport, // 是否支持Xuất khẩu文件
          ...
          onEditableSave: options.onEditableSave, // 行内Sửa的事件
          ...
          onLoadSuccess: $.table.onLoadSuccess, // 当tất cả数据被加载时触发处理函数
          exportOptions: options.exportOptions, // 前端Xuất khẩu忽略列索引
          detailFormatter: options.detailFormatter, // 在行下面展示其他数据Danh sách
        });
      },
      ...
      onLoadSuccess: function (data) {
        ...
        $('.table [data-toggle="popover"]').each(function () {
          $(this)
            .popover({ trigger: "manual", html: true, animation: false, container: "body", placement: "left" })
    ...
    treeTable: {
      init: function (options) {
        ...
        $.bttTable = $("#" + options.id).bootstrapTreeTable({
          ...
          type: "post", // 请求方式（*）
          url: options.url, // 请求后台的URL（*）
          data: options.data, // Khôngurl时用于渲染的数据
          ...
          columns: options.columns, // 显示列信息（*）
          responseHandler: $.treeTable.responseHandler, // 当tất cả数据被加载时触发处理函数
        });
      },
    ...
    operate: {
      ...
      addGroup: function (id) {
        table.set();
        $.modal.open("Thêm " + table.options.modalName, $.operate.addUrl(id), 700, 440);
      },
      ...
      editTreeList: function (id) {
         table.set();
        var rowsEdit = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
         ...
         var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
         url = table.options.updateUrl.replace("{rowsEdit}",rowsEdit);
         $.modal.open("Sửa " + table.options.modalName, url);
        } else {
        var url = $.operate.editUrl(id).replace("{rowsEdit}",rowsEdit);
        $.modal.open("Sửa " + table.options.modalName,url);
       }
      },
      editGroup: function (id) {
        ...
        if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
          ...
          var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
          $.modal.open("Chỉnh Sửa " + table.options.modalName, url, 700, 380);
        } else {
          $.modal.open("Chỉnh Sửa " + table.options.modalName, $.operate.editUrl(id), 700, 440);
        }
      },
      ...
      successTabCallback: function (result) {
        if (result.code == web_status.SUCCESS) {
          ...
          var $contentWindow = $('.apm_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
          $.modal.close();
